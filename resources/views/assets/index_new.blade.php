...
@section('content')
<div class="p-6" x-data="assetManager()">
    <!-- Header Section -->
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden card-hover">
                ...
                <div class="group bg-white border border-gray-200 rounded-xl p-6 hover:border-indigo-300 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                        ...
                        <div class="opacity-0 group-hover:opacity-100 transition-opacity">
                            <div class="relative" x-data="{ menuOpen: false }">
                                ...
                                <div x-show="menuOpen"
                                    ...
                                    @if(!empty($asset->id))
                                    <a href="{{ route('assets.show', ['asset' => $asset->id]) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                        <i class="fas fa-eye mr-2"></i>Lihat Detail
                                    </a>
                                    @endif
                                    @if(!empty($asset->id))
                                    <a href="{{ route('assets.edit', ['asset' => $asset->id]) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                        <i class="fas fa-edit mr-2"></i>Edit
                                    </a>
                                    @endif
                    @if(!empty($asset->id))
                    <button onclick="confirmDelete('{{ route('assets.destroy', ['asset' => $asset->id]) }}')"
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex space-x-2 pt-3">
                            @if(!empty($asset->id))
                            <a href="{{ route('assets.show', ['asset' => $asset->id]) }}"
        ...
        <!-- List View -->
        <div x-show="viewMode === 'list' && !loading" class="overflow-x-auto">
